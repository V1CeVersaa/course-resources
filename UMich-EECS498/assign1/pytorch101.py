...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    start = start if start % 10 == 0 else (start // 10 + 1) * 10
    stop = stop if stop % 10 == 0 else (stop // 10 + 1) * 10
...
def shuffle_cols(x: Tensor) -> Tensor:
    ...
    idx_y = torch.tensor([0, 1, 2, 3])
    idx_x = torch.tensor([0, 0, 2, 1])
    y[:, idx_y] = x[:, idx_x]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = torch.zeros(3)
    idx_x = torch.tensor([1, 0, 3])
    idx_y = torch.tensor([0, 1, 2])
    y = x[idx_x, idx_y]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def make_one_hot(x: list[int]) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    len_x, len_y = len(x), max(x) + 1
    idx = torch.arange(len_x)
    y = torch.zeros(len_x, len_y, dtype=torch.float32)
    y[idx, x] = 1
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statem
    y = x.clone()
    y[torch.arange(y.shape[0]), x.argmin(dim=1)] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    means = x.mean(dim=0)
    stds = x.std(dim=0)
    y = (x - means.view(1, -1)) / stds.view(1, -1)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
