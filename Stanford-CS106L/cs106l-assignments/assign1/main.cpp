...
void parse_csv(std::string filename, std::vector<Course> &courses) {
    ...
    if (input_file.is_open()) {
        std::string in_line;
        std::getline(input_file, in_line);
        while (std::getline(input_file, in_line)) {
            auto splited_line = split(in_line, ',');
            Course parsed_course{splited_line[0], splited_line[1],
                                 splited_line[2]};
            courses.emplace_back(parsed_course);
        }
        input_file.close();
    }
}
...
void write_courses_offered(std::vector<Course> &all_courses) {
    ...
    std::vector<Course> needed_deleted;
        ...
        for (const auto &course : all_courses) {
            if (course.quarter != "null") {
                output_file << course.title << "," << course.number_of_units
                            << "," << course.quarter << "\n";
                needed_deleted.push_back(course);
            }
        }
        output_file.close();
    }

    for (const auto &course : needed_deleted) {
 ...
 * "student_output/courses_not_offered.csv".
 *
 * This function is ALWAYS called after the `write_courses_offered`
 * function. `unlisted_courses` will trivially contain classes that are not
 * offered since you delete offered classes from `all_courses` in the
