...
class CourseDatabase {
   public:
    CourseDatabase(std::string filename) {
        ...
        std::transform(lines.begin(), lines.end(), std::back_inserter(courses), [](std::string line) {
    ...
    std::optional<Course> find_course(std::string course_title) {
        ...
        auto it = std::find_if(courses.begin(), courses.end(),
                               [&course_title](const Course &course) { return course.title == course_title; });
...
int main(int argc, char *argv[]) {
    static_assert(
        !std::is_same_v<std::invoke_result_t<decltype(&CourseDatabase::find_course), CourseDatabase, std::string>,
                        FillMeIn>,
