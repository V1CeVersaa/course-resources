...
void Token::clean(const std::string &source) {
    ...
    auto right = std::find_if(std::make_reverse_iterator(end), std::make_reverse_iterator(begin), ::isalnum).base();
...
size_t levenshtein(const std::string &s1, const std::string &s2) {
    ...
    /* If strings have same size, we'll count mismatches and potentially early
     * exit */
    if (diff == 0) {
        size_t mismatches =
            std::inner_product(s1.begin(), s1.end(), s2.begin(), 0UL, std::plus<>(), std::not_equal_to<>());
