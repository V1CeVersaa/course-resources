...
void print_success() {
    std::vector<std::string> messages = {"You're a spelling wizard! If words had a kingdom, you'd rule it.",
...
int main(int argc, char **argv) {
    ...
    if (read_stdin) {
        auto iters = read_stream_iterator(std::cin);
        input += std::string(iters[0], iters[1]);
    }
    ...
    auto iters = read_stream_iterator(dict_stream);
    std::string dict_contents(iters[0], iters[1]);
    ...
    if (styled && !dictionary.empty() && mispellings.empty()) print_success();
